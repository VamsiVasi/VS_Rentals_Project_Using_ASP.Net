...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:rentConnectionString %>" ProviderName="<%$ ConnectionStrings:rentConnectionString.ProviderName %>" SelectCommand="SELECT productid, picture, productname, producttype, description, availstate, availcity, rent, status FROM products WHERE (vendorname = @vendorname)">
        <SelectParameters>
   ...
   <asp:GridView ID="GridView2" runat="server" AllowPaging="True"
    ...
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:rentConnectionString %>" ProviderName="<%$ ConnectionStrings:rentConnectionString.ProviderName %>" SelectCommand="SELECT booking.productid, products.picture, products.productname, products.producttype, booking.bname, booking.bemail, booking.bphnumber, booking.bnumpieces, booking.pdate, booking.ldate, products.availstate, products.availcity, booking.pplace FROM booking INNER JOIN products ON booking.productid = products.productid WHERE (booking.ldate &gt; CURRENT_DATE) AND (products.vendorname = @vendorname)">
        <SelectParameters>
